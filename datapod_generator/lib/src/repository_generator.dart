...
class RepositoryGenerator extends GeneratorForAnnotation<api.Repository> {
  ...
  String _generateOperationsImpl(
          ...
          TypeReference(
            (tr) => tr
              ...
              ..types.addAll([
                ...
                refer(keyType.getDisplayString()),
              ]),
          ),
        )
        ..fields.addAll([
          ...
          Field(
            (f) => f
              ...
              ..assignment = Code(
                "'''${SqlGenerator.generateInsert(metadata)}'''",
              ),
          ),
          Field(
            (f) => f
              ...
              ..modifier = FieldModifier.constant
              ..assignment = Code(
                "'''${SqlGenerator.generateUpdate(metadata)}'''",
              ),
          ),
          Field(
            (f) => f
              ...
              ..modifier = FieldModifier.constant
              ..assignment = Code(
                "'''${SqlGenerator.generateDelete(metadata)}'''",
              ),
          ),
          Field(
            (f) => f
              ...
              ..modifier = FieldModifier.constant
              ..assignment = Code(
                "'''${SqlGenerator.generateFindById(metadata)}'''",
              ),
          ),
          Field(
            (f) => f
        ...
        ..constructors.add(
          Constructor(
            (c) => c
              ...
              ..initializers.add(
                refer('super').call([refer('relationshipContext')]).code,
              )
              ..body = Block.of([
                Code(
                  'relationshipContext.registerOperations<${entityClass.name}, ${keyType.getDisplayString()}>(operations);',
                ),
                Code(
                  'relationshipContext.registerMapper<${entityClass.name}>(mapper);',
                ),
              ]),
          ),
        )
  ...
  Method _generateDeleteMethod(
    ...
    final cascadeRemove = metadata.columns
        .where((c) => c.cascadeRemove)
        .toList();
  ...
  Method _generateFindByIdMethod(
    ...
    final fetchJoinAnns = method != null
        ? TypeChecker.fromUrl(
            'package:datapod_api/annotations.dart#FetchJoin',
  ...
  Code _generateRepoReturnBody(
    ...
    if (type == 'findById') {
      ...
      for (final ann in fetchJoinAnns) {
        final property = ann.getField('property')?.toStringValue();
        if (property != null) {
          final col = metadata.columns.firstWhere(
            (c) => c.fieldName == property,
          );
          if (col.relationType == 'ManyToOne' ||
              (col.relationType == 'OneToOne' && col.columnName.isNotEmpty)) {
            final entityField =
                (method!.enclosingElement as ClassElement).allSupertypes
                        .firstWhere((s) => s.element.name == 'BaseRepository')
                        .typeArguments[0]
                        .element
                    as ClassElement;
            final relationField = entityField.fields.firstWhere(
              (f) => f.name == property,
            );
            final relatedType =
            ...
            for (final c in relatedMetadata.columns) {
              if (c.columnName.isNotEmpty) {
                joinSelects.add(
                  't$jIdx.${c.columnName} AS t${jIdx}_${c.columnName}',
                );
              }
            }
    ...
    return Block.of([Code(sqlCode.toString()), Code(mappingCode.toString())]);
  }
      ...
      (m) => m
        ...
        ..returns = TypeReference(
          (tr) => tr
            ..symbol = 'Future'
            ..types.add(
              refer('QueryResult', 'package:datapod_api/datapod_api.dart'),
            ),
        )
        ..modifier = MethodModifier.async
      ...
      (m) => m
        ...
        ..requiredParameters.add(
          Parameter(
            (p) => p
              ..name = 'id'
              ..type = refer(keyType.getDisplayString()),
          ),
        )
        ..body = Block.of([
  ...
  Iterable<Method> _generateOperationsQueryResultMethods(
    ...
    for (final method in allMethods) {
      ...
      if ((method.name!).startsWith('findBy') ||
          ...
          (method.name!).startsWith('streamBy')) {
    ...
    for (final col in metadata.columns.where(
      (c) => c.relationType != null && c.columnName.isNotEmpty,
    )) {
      final methodName =
  ...
  Method _generateOperationsDslMethod(
    ...
    final sortChecker = TypeChecker.fromUrl(
      'package:datapod_api/pagination.dart#Sort',
    );
    ...
    final otherParams = method.formalParameters
        .where(
          (p) =>
    ...
    final fetchJoinAnns = TypeChecker.fromUrl(
      'package:datapod_api/annotations.dart#FetchJoin',
    ).annotationsOf(method);
    ...
    return Method(
      (m) => m
        ...
        ..body = Block.of([
          Code(
            'final params = ${_generateDslParamMap(metadata, components, otherParams)};',
          ),
          Code(
  ...
  Iterable<Method> _generateRepoQueryResultMethods(
    ClassElement repoInterface,
    ClassElement entityClass,
    EntitySqlMetadata metadata,
  ) {
  ...
  Method _generateRepoDslMethod(
    ...
    final type = method.name!.startsWith('count')
        ...
        ? 'delete'
        : 'find';
    final dsl = type == 'find' && !method.name!.startsWith('find')
        ? DSLParser.parse('findBy')
    ...
    int paramIdx = 0;
    for (final comp in dsl) {
      // Changed from `components` to `dsl`
      if (comp.operator == 'IsNull' ||
          comp.operator == 'IsNotNull' ||
          comp.operator == 'IsEmpty' ||
          comp.operator == 'IsNotEmpty' ||
    ...
    final isPage =
        returnTypeStr.startsWith('Future<Page<') ||
        ...
        ..body = Block.of([
          if (isPage) ...[
            if ((method.name!) == 'findAllPaged')
              ...
              Code(
                'final result = ${isStream ? '' : 'await '}operations.${method.name!}(${method.formalParameters.map((p) => p.isNamed ? '${p.name!}: ${p.name!}' : p.name!).join(', ')});',
              ),
            Code(
              "final totalElements = await operations.database.connection.execute(",
            ),
            Code(
              "  applyPagination('''${SqlGenerator.generateDslQuery(metadata, dsl, 'count', dslParamNames)}''', fieldToColumn: ${repoInterface.name}OperationsImpl._fieldToColumn),", // Changed from `components` to `dsl`
            ),
            Code(
              '  ${_generateDslParamMap(metadata, dsl, otherParams)}',
            ), // Changed from `components` to `dsl`
            Code(');'),
            Code('return Page('),
            Code(
      ...
      if (comp.operator == 'IsNull' ||
        ...
        if (paramIdx < otherParams.length) {
          ...
          if (column.converterType != null) {
            val =
                'const ${column.converterType}().convertToDatabaseColumn($val)';
          }
  ...
  Code _generateRepoDslReturn(
    ...
    switch (type) {
      ...
      case 'exists':
        return Code(
          'final val = result.rows.first.values.first; return val is bool ? val : val != 0;',
        );
      case 'delete':
        return Code('return;');
      default:
  ...
  Method _generateFindAllPagedMethod(
    ClassElement repoInterface,
    ClassElement entityClass,
    EntitySqlMetadata metadata,
  ) {
